...
export default {
    ...
    toggle() {
      ...
      if(isActive) {
        atom.notifications.addInfo('Autosave Onchange Plus Enabled!', {
          text: '',
          dismissable: true
        });
      } else {
        atom.notifications.addInfo('Autosave Onchange Plus Disabled!', {
          text: '',
          dismissable: true
        });
      }
    ...
    activate(state) {
        ...
        this.subscriptions.add(
          atom.commands.add('atom-workspace', {
            'autosave-onchange-plus:toggle': this.toggle
          })
        );

        atom.config.onDidChange('autosave-onchange-plus.delay', (values) => {
          console.log(`autosave delay changed from ${values.oldValue} to ${values.newValue}`);
          saveDelay = values.newValue;
        });

        saveDelay = atom.config.get('autosave-onchange-plus.delay');
